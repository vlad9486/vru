use vru_noise::{SymmetricState, ChainingKey, Key, Cipher, Rotor};
...
pub fn out0(
    ...
    (InitiatorsEphemeral { symmetric_state, e_pk, e_sk }, e_pkc)
}
    ...
    EncryptedDefault<PublicKeyBytes>: Line,
{
    ...
    let InitiatorsEphemeral { symmetric_state, e_pk, e_sk } = state;
    ...
    Ok((
        ...
        peer_s_pk,
        payload_p,
        (Concat(peer_e_pq.ct, payload_q), peer_s_pq.ct, Concat(enc_s_pkc, payload_r)),
    ))
}
    ...
    Z: Rotor<Noise>,
{
    ...
    let (cipher, hash) = symmetric_state
        .finish();
    ...
    EncryptedDefault<PublicKeyBytes>: Line,
{
    ...
    (
        RespondersEphemeral { symmetric_state, e_sk, e_pk },
    ...
    Z: Rotor<Noise>,
{
    ...
    let RespondersEphemeral { symmetric_state, e_pk, e_sk } = state;

    let (symmetric_state, payload_q) = symmetric_state
        .mix_shared_secret(&e_pk.decapsulate(&e_sk, &peer_e_ct))
        .decrypt_line::<Q>(payload_q)
    ...
    Ok((cipher.swap(), hash, peer_s_pk, payload_q, payload_r, Concat(peer_s_pq.ct, payload_s)))
}
